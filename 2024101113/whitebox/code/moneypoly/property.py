"""Module containing Property and PropertyGroup classes for MoneyPoly."""
...
class PropertyConfig:
    """Holds the static configuration data for a property tile."""

    name: str
    position: int
    price: int
    base_rent: int
    group: Optional[object] = field(default=None)
...
class Property:
    ...
    def __init__(self, cfg):
        """Initialise from a PropertyConfig instance."""
        self.config = cfg
        self.owner = None
        ...
        if cfg.group is not None and self not in cfg.group.properties:
            cfg.group.properties.append(self)

    # ------------------------------------------------------------------
    # Convenience proxies so existing code still reads property.name etc.
    # ------------------------------------------------------------------
    ...
    @property
    def base_rent(self):
        """Return the base rent before any multiplier."""
        return self.config.base_rent

    @property
    def mortgage_value(self):
        """Return the mortgage payout (half the purchase price)."""
        return self.config.price // 2

    @property
    def group(self):
        """Return the PropertyGroup this property belongs to."""
...
def create_property(name, position, price, base_rent, group=None):
    """Factory that builds a Property from individual fields (used by Board)."""
    return Property(PropertyConfig(name, position, price=price, base_rent=base_rent, group=group))
